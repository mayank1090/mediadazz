...
const BusinessCategory = ({ isLoggedIn, businessInsights = [] }: BusinessCategoryProps) => {
  return (
  ...
  {businessInsights.length > 0 ? (
    businessInsights.map((insight, index) => (
      <div key={index} className="p-6  border border-[#EEEEEE] bg-white rounded-[0.625rem] flex items-center gap-4 lg:gap-[1.125rem]">
        <div className="p-2.5 rounded-full overflow-hidden bg-[#FFEAE1] shrink-0">
          <LiaCarSolid className=' text-brand w-6 h-6 shrink-0'/>
        </div>
        <div className="">
          <h3 className="pb-1 font-satoshi font-bold text-xs md:text-base lg:text-lg">{insight.business_type}</h3>
          <p className="text-[0.625rem] font-satoshi md:text-sm text-[#6B7280] font-medium lg:text-base ">{insight.reason}</p>
        </div>
      </div>
    ))
  ) : (
