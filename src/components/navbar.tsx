		...
		<header className="sticky top-0 z-50 bg-white overflow-x-clip">
			<nav aria-label="Primary" className="mx-auto max-w-[1330px] p-6 lg:px-6 xl:px-8 lg:py-8">
				<div className="flex h-10 lg:h-12 items-center justify-between">
					...
					<Link href="/" className="h-6 lg:h-7 max-w-[80%] lg:max-w-full" aria-label="Go to homepage">
					...
					<ul className="hidden lg:flex lg:relative items-center gap-4 xl:gap-8">
						...
						{topLevels.map((t, i) => (
							...
							>
								...
								>
									<span className={i === clickedIndex ? 'text-brand font-satoshi font-bold text-sm xl:text-base' : ''}>{t.label}</span>
									<FiChevronDown className={`h-4 w-4 xl:h-6 xl:w-6 transition-transform ${clickedIndex === i ? 'rotate-180' : ''}`} />
								</button>
										...
										<div className="grid grid-cols-1 md:grid-cols-3 gap-8 p-8">
											{t.sections.map((sec) => (
												<div key={sec.heading} className="min-w-0">
													<h3 className="text-sm xl:text-base font-medium font-satoshi text-brand uppercase mb-6">{sec.heading}</h3>
